...
class TaylorAttn(ParallelLayer):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        DEBUG = False
        PROFIILE_TIME = False
        if DEBUG: print(f"forward step(-1), input_.shape: {input_.shape}")
        ...
        if DEBUG: print(f"forward step(0), input_.shape: {input_.shape}")
        
        B, N, C = input_.shape # Batch, Seq_len, Channels
        ...
        if DEBUG: print(f"forward step(1), k_hat.shape: {k_hat.shape}")
        
        # Step 2: Global context matrix
        g = k_hat.transpose(-2, -1) @ v
        if DEBUG: print(f"forward step(2), g.shape: {g.shape}")
        ...
        if DEBUG: print(f"forward step(3), g: {g.shape}, k_sum:{k_sum.shape}, v_sum:{v_sum.shape}")
        
        # Step 4: Compute Taylor denominator
        q_ksum_T = q @ k_sum.transpose(-2, -1)
        temp = N * self.scale
        t_D = torch.ones_like(q_ksum_T, device=q.device) * temp + q_ksum_T
        if DEBUG: print(f"forward step(4), t_D.shape: {t_D.shape}")
        
        
        # Step 5: Compute Taylor numerator
        T_N = torch.ones((B, q.shape[1], N, 1), device=q.device) * self.scale + q @ g
        if DEBUG: print(f"forward step(5), T_N.shape: {T_N.shape}")
        ...
        if DEBUG: print(f"forward step(6), z_.shape: {z.shape}")
